...
class RankBasedRPNHead(nn.Module):
    """
    ...
    def __init__(self, in_channels, num_anchors，head_weight=0.20,  **kwargs):
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def compute_loss(self, objectness, pred_bbox_deltas, labels, regression_targets):
        ...
        pos_weights = regression_targets.detach().sigmoid().max(dim=1)[0][sampled_pos_inds]

        bbox_avg_factor = torch.sum(pos_weights)
        if bbox_avg_factor < 1e-10:
            bbox_avg_factor = 1

        box_loss = torch.sum(pos_weights*box_loss)/bbox_avg_factor
